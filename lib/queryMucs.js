const MESSAGE_MARK_METHOD = "XMPP::Message::mark";
const INTERACTION_CLAIM_METHOD = "XMPP::Message::claim_interaction";
const INTERACTION_UNCLAIM_METHOD = "XMPP::Message::unclaim_interaction";
const INTERACTION_CLOSE_METHOD = "XMPP::Message::close_interaction";
...
module.exports = function (client, stanzas) {
    ...
    var Room = stanzas.define({
        ...
        fields   : {
            ...
            interaction: Utils.boolAttribute('interaction'),
        }
    });
    ...
    var UniserverResponse = stanzas.define({
        name     : 'uniserverResponse',
        element  : 'result',
        namespace: 'xmpp:thirdlane:uniserver',
        fields   : {
            status: Utils.textSub('xmpp:thirdlane:uniserver', 'status')
        }
    });

    stanzas.extendIQ(UniserverResponse);

    client.sendUniserverAction = function (command, cb) {
        return this.sendIq({
            to       : 'ami-proxy.default.local',
            type     : 'set',
            uniserver: {
                command: command
            }
    ...
    stanzas.withPresence(function (PRESENCE) {
        stanzas.add(PRESENCE, 'os', Utils.subAttribute('xmpp:thirdlane:presnece:os', 'os', 'value'));
        stanzas.add(PRESENCE, 'version', Utils.subAttribute('xmpp:thirdlane:presnece:version', 'version', 'value'));
        stanzas.add(PRESENCE, 'platform', Utils.subAttribute('xmpp:thirdlane:presnece:platform', 'platform', 'value'));
        stanzas.add(PRESENCE, 'platformVersion', Utils.subAttribute('xmpp:thirdlane:presnece:platform_version', 'platform_version', 'value'));
        stanzas.add(PRESENCE, 'timeZone', Utils.subAttribute('xmpp:thirdlane:presnece:time_zone', 'time_zone', 'value'));
        stanzas.add(PRESENCE, 'onPhone', Utils.boolSubAttribute('xmpp:thirdlane:presnece:on_phone', 'on_phone', 'value'));
    });
    ...
    client.on('message', function (msg) {
        if (msg.server_received) {
            client.emit('message:server_received', msg);
        }
    });
    ...
    client.claimInteraction = function (params) {
        var msg = {
            method: INTERACTION_CLAIM_METHOD,
            params: {
    ...
    client.unclaimInteraction = function (params) {
        var msg = {
            method: INTERACTION_UNCLAIM_METHOD,
            params: {
                from   : client.jid.bare,
                to     : params.to.bare,
                mid    : params.mid,
    ...
    client.closeInteraction = function (params) {
        var msg = {
            method: INTERACTION_CLOSE_METHOD,
            params: {
                from   : client.jid.bare,
                to     : params.to.bare,
                mid    : params.mid,
    ...
    client.markMessageAsRead = function (params) {
        ...
        if (params.read_mark) {
            msg.params.read_mark = params.read_mark
        } else {
            msg.params.mid = params.mid || ''
            msg.params.is_read = 1
        }

        if (params.interaction_id) {
            msg.params.interaction_id = params.interaction_id
        }
    ...
    stanzas.withMessage(function (MESSAGE) {
        stanzas.add(MESSAGE, 'unread_sync', Utils.textSub('xmpp:thirdlane:unread_sync', 'payload'));
    });

    client.on('message', function(msg) {
        if (msg && msg.unread_sync) {
            try {
                msg.unread_sync = JSON.parse(msg.unread_sync);
                client.emit('unread_sync',  msg);
            } catch (e) {
                console.error('Failed to parse unread_sync payload, malformed JSON')
            }
        }
    });
